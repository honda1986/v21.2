...
def select_bets_by_ev(combo_probs, odds_map, ev_th, min_prob, max_n) -> List[Dict]:
    out = []
    for combo, p in combo_probs.items():
        o = odds_map.get(combo, 0.0)
        if o <= 0 or p < min_prob: continue
        ev = p * o
...
min_prob = st.sidebar.slider("最低予想確率(%)", 0.0, 20.0, 3.0, 0.5) / 100.0
max_bets = st.sidebar.slider("1レース上限点数", 1, 12, 4, 1)
bet_amt  = st.sidebar.number_input("1点の購入金額(円)", min_value=100, step=100, value=100)
st.sidebar.caption("💡 補正ONなら EV閾値 1.10〜1.30 で見送りが増えるはず。"
                   "全レース買い目が出るなら閾値・最低確率を上げる。")
...
with tab2:
    ...
    if st.button("🔍 取得して予想", type="primary", use_container_width=True):
        ...
        if odds_map:
            chosen = select_bets_by_ev(combo_probs, odds_map, ev_th, min_prob, max_bets)
            st.subheader(f"🎯 採用買い目 (補正{'ON' if use_calib else 'OFF'}, EV≥{ev_th}, 確率≥{min_prob*100:.1f}%)")
            if chosen:
                df_b = pd.DataFrame([{
                    "買い目": c["bet"],
